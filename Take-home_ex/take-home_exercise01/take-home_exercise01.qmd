...
We will use the ggstats package (function gglikert) because it works directly with your tidy data frame and is highly customizable.

Prepare the Data Summary

```{r}
# 1. Prepare the Data (Counts)
# We keep counts here so HH can calculate the totals for us
...
```{r}
# 1. Summarize Data for Labels
# We calculate percentages first so we can add text labels to the bars
...
Perform a Chi-Square Test of Independence. This will tell us if the relationship between Gender and GPA is statistically significant ($p < 0.05$).


```{r}
# --- Observation 3: Statistical Confirmation of Gender Performance ---

# We use ggbarstats to test the association between Gender and GPA
ggbarstats(
  data = students_clean,
  x = GPA,           # The Dependent Variable (Grades)
  y = Gender,        # The Independent Variable (Group)
...
```{r}
# --- Observation 4: Study Intensity across GPA Levels ---

students_clean %>%
  # Ensure Time_Studying is numeric for the density plot calculation
  ...
  # Map numeric breaks to the actual hour strings from your Codebook
  scale_x_continuous(
    breaks = 1:5,
