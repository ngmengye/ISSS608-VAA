...
Using classification method = quantile
...
tm_shape(mpsz_pop2020[mpsz_pop2020$REGION_N=="CENTRAL REGION", ]) +  
  tm_fill("DEPENDENCY",           
          ...
          legend.hist = TRUE) +  
  tm_layout(
    main.title = "Dependency Ratio - Central Region",
    legend.outside = TRUE,            
    legend.outside.position = "right",
    legend.hist.height = 0.3,         # Fixes the overlap by limiting hist height
...
#| message: false
#| warning: false
percentmap <- function(vnam, df, legtitle = "Percentile", mtitle = "Percentile Map"){
  
  # Calculate percentiles
  percent <- c(0, .01, .1, .5, .9, .99, 1)
  var <- df[[vnam]] # Cleaner way to get variable in v4
  ...
  tm_shape(df) +
    tm_polygons(
      ...
      fill.scale = tm_scale_intervals(
        breaks = bperc,
        values = "brewer.blues", # v4 uses 'values' and 'brewer.blues'
        labels = c("< 1%", "1% - 10%", "10% - 50%", "50% - 90%", "90% - 99%", "> 99%")
      ),
      fill.legend = tm_legend(title = legtitle) # New way to set legend title
    ) +
    tm_borders() +
    tm_title(mtitle) + # Use tm_title instead of tm_layout(main.title)
    tm_layout(legend.position = c("right", "bottom"))
}
```
...
boxmap <- function(vnam, df, 
                   legtitle = "Classification", 
                   mtitle = "Box Map", 
                   mult = 1.5) {
  
  # 1. Get variable and calculate breaks
  var <- df[[vnam]]
  bb <- boxbreaks(var, mult = mult) # Ensure your boxbreaks function is defined
  ...
  tm_shape(df) +
    tm_polygons(
      ...
      fill.scale = tm_scale_intervals(
        breaks = bb,
        values = "brewer.blues", # Fixes the palette warning from image_251d82
        labels = c("lower outlier", "< 25%", "25% - 50%", "50% - 75%", "> 75%", "upper outlier")
      ),
      fill.legend = tm_legend(title = legtitle) # Fixes the "length > 1" error
    ) +
    tm_borders() +
    # 2. Fix the title overlap
    tm_title(mtitle, size = 1.2) + 
    tm_layout(
      frame = FALSE,              # Removing the frame prevents title squashing
      legend.outside = TRUE,      # Moves legend out of the map space
      legend.outside.position = "right"
    )
}
